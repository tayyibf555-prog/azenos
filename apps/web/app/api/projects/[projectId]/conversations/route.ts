...
import { conversationsQuerySchema, getProjectConversations } from "./query";
...
export const GET = withErrorHandling(async (req: Request, { params }: Ctx) => {
  ...
  const parsed = conversationsQuerySchema.safeParse(searchParamsObject(req));
  ...
  const data = await getProjectConversations(orgId, projectId, parsed.data);
