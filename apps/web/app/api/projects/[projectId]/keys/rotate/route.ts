...
export const POST = withErrorHandling(async (_req: Request, { params }: Ctx) => {
  ...
  const rotated = await rotateActiveKey(orgId, projectId);
  if (!rotated) return jsonError(404, "no_active_key");
  // Plaintext secret shown once — same public key, old secret already dead.
