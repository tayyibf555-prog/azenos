...
import { getProjectCosts } from "../../../../../lib/server/queries";
import {
  isUuid,
  monthQuerySchema,
  searchParamsObject,
...
export const GET = withErrorHandling(async (req: Request, { params }: Ctx) => {
  ...
  const costs = await getProjectCosts(orgId, projectId, parsed.data.month);
  if (costs === null) return jsonError(404, "project_not_found");
